...
class VideoPlayer:
    def __init__(self, opt) -> None:
        ...
        if opt.csv_dir is None:
            ...
            self.csv_path = Path(opt.csv_dir) / Path(self.video_path.stem).with_suffix(
                ".csv"
            )
        ...
        if os.path.exists(self.csv_path):
            ...
            if len(self.info.index) != self.frames:
                print(
                    "pd len: {}, camera len: {}".format(
                        len(self.info.index), self.frames
                    )
                )
                ...
                self.info = {
                    k: list(v.values()) for k, v in self.info.to_dict().items()
                }
    ...
    def save_piece(self):
        ...
        out = cv2.VideoWriter(
            "{}_{}.mp4".format(self.piece_start + 1, self.piece_end + 1),
            self.fourcc,
            self.fps,
    ...
    def display(self):
        ...
        res_frame = cv2.putText(
            res_frame,
            state_name[self.info["visible"][self.frame_num]],
            (100, 110),
        ...
        res_frame = cv2.putText(
            res_frame,
            "Frame: {}, Total: {}".format(int(self.frame_num + 1), int(self.frames)),
            (100, 50),
            cv2.FONT_HERSHEY_SIMPLEX,
            2,
            (0, 0, 255),
        ...
        res_frame = cv2.putText(
            res_frame,
            "Piece: {}-{}".format(int(self.piece_start + 1), int(self.piece_end + 1)),
            (100, 170),
            cv2.FONT_HERSHEY_SIMPLEX,
            2,
            (0, 0, 255),
    ...
    def main_loop(self):
        ...
        if key in keybindings["first_frame"]:
            ...
            self.cap.set(
                cv2.CAP_PROP_POS_FRAMES, self.frame_num
            )  # cap.set时, frame_num不用加1
            ret, self.frame = self.cap.read()
    ...
    def finish(self):
        ...
        df = pd.DataFrame.from_dict(self.info).sort_values(
            by=["frame_num"], ignore_index=True
        )
...
def parse_opt():
    ...
    parser.add_argument(
        "video_path", type=str, nargs="?", default=None, help="Path to the video file."
    )
    parser.add_argument(
        "--csv_dir",
        type=str,
...
def remove_duplicate_frames(video_path, output_path):
    ...
    while True:
        ...
        if previous_frame is None or cv2.PSNR(frame, previous_frame) < 40.0:
            out.write(frame)
...
if __name__ == "__main__":
    ...
    if opt.video_path is None:
        ...
        toRemove = input(
            'Should duplicated, consecutive frames be deleted? Insert "y" or "n": \n'
        )
        if toRemove == "y":
            bez_duplikatow_video_path = str(
                video_path.with_stem(video_path.stem + "_no_dups")
            )
