...
let frameDetections = {};   // { frameNum(number): {source, detections:[{class_id,class_name,score,box,keypoints:[[x,y],...]}]} }
let stabilizedDetections = []; // [{class_id,class_name,point_index,xy:[x,y],n_frames}, ...] (動画全体の統計的代表座標。1クラス内の各スロットごとに1件)
let useStabilized = false;
let showSlotLabel = true;
let currentFrame = 0;
let confThres = 0.3;
...
function drawDetections(n) {
  if (useStabilized) {
    for (const d of stabilizedDetections) {
      drawOnePoint(d.xy, d.class_id, false, d.point_index);
    }
    return;
  }
  ...
  for (const d of entry.detections) {
    if (d.score < confThres) continue;
    (d.keypoints || []).forEach((xy, slot) => {
      drawOnePoint(xy, d.class_id, false, slot);
    });
  }
}

function drawOnePoint(xy, classId, outline, slot) {
  const [cx, cy] = xy;
  ...
  if (showSlotLabel && slot !== undefined) {
    ctx.fillStyle = '#fff';
    ctx.font = 'bold 11px sans-serif';
    ctx.fillText(String(slot), cx + 8, cy - 8);
  }
}
...
function onSlotLabelToggle(checked) {
  showSlotLabel = checked;
  if (currentVideo) renderFrame(currentFrame);
}
