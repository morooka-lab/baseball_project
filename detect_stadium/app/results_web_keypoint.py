"""
キーポイント検出モデル(train_keypoint.py)の推論結果をブラウザ上で確認する
ビューア(閲覧専用、編集不可)。app/results_web.py(SSDlite用)と同じ方式だが、
infer_keypoint.py / models/keypoint_detector.py / utils/keypoint_stabilize.py を使う。

使い方:
    python app/results_web_keypoint.py --task stadium \
        --weights runs/train_keypoint/stadium/exp1/best.pt
    python app/results_web_keypoint.py --task stadium \
        --weights runs/train_keypoint/stadium/exp1/best.pt --conf-thres 0.05 --port 5031
"""
...
# ── PyAVコンテナpool(bbox_label_web.py/results_web.pyと同じ方式) ────────────
_caps: dict = {}
_cap_locks: dict = {}
...
def get_or_run_inference(video_path: str) -> dict:
    ...
    with _infer_lock:
        ...
        if cpath.exists():
            with open(cpath, "r", encoding="utf-8") as f:
                return json.load(f)
        result = run_on_video(MODEL, Path(video_path), CLASSES, POINT_COUNTS, DEVICE,
                               CONF_THRES, frame_stride=1)
...
def detections():
    ...
    result["stabilized"] = aggregate_video_keypoints(
        result["frames"], result["classes"], STABILIZE_CONF_THRES
    )
...
def parse_args():
    p = argparse.ArgumentParser(
        description="キーポイント検出モデルの推論結果を動画で確認するビューア(閲覧専用)",
    ...
    p.add_argument("--out-dir", default=None,
                    help="推論結果JSONのキャッシュ先 (default: runs/detect_keypoint/<task>/<weightsのrun名>)")
...
if __name__ == "__main__":
    ...
    MODEL = load_checkpoint(args.weights, len(CLASSES) + 1, meta["num_keypoints"], device=DEVICE)
