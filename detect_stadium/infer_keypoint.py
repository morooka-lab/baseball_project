"""
学習済みキーポイント検出モデルで未ラベル動画に推論する。infer.py(SSDlite)とは
別の並行パイプライン。

フレームごとの検出結果(box + confidence + keypoints)をJSONに保存する。
推論時のkeypointsのvisibility値は常に1になる(モデル側にパディングスロットか
どうかの情報が無いため)。そのため、predicted labelから逆算した
point_counts[class_name]個までをそのクラスの「実点」としてトリムして出力する。
...
使い方:
    python infer_keypoint.py --task stadium --weights runs/train_keypoint/stadium/exp/best.pt \
        --source /data2/baseball_data/detect_dataset/videos --save-video
"""
...
def run_on_video(model, video_path: Path, classes, point_counts, device, conf_thres,
                  frame_stride, save_video_path=None):
    ...
    for av_frame in container.decode(stream):
        ...
        if idx % frame_stride == 0:
            ...
            dets = []
            for box, label, score, kps in zip(pred["boxes"].tolist(), pred["labels"].tolist(),
                                                pred["scores"].tolist(), pred["keypoints"].tolist()):
                if score < conf_thres:
                    continue
                ...
                kps_real = [[round(x, 1), round(y, 1)] for x, y, _v in kps[:n_real]]
                dets.append({
                    "class_id": class_id,
                    "class_name": name,
                    "score": round(score, 4),
            ...
            if writer is not None:
                for d in dets:
                    ...
                    for slot, (kx, ky) in enumerate(d["keypoints"]):
                        cv2.circle(frame, (int(kx), int(ky)), 4, (0, 255, 255), -1)
                        cv2.putText(frame, str(slot), (int(kx) + 5, int(ky) - 5),
...
def parse_args():
    p = argparse.ArgumentParser(description="学習済みキーポイント検出モデルによる推論")
    ...
    p.add_argument("--out-dir", default=None,
                    help="JSON出力先 (default: runs/detect_keypoint/<task>/<weightsのrun名>)")
    p.add_argument("--conf-thres", type=float, default=0.3)
    p.add_argument("--frame-stride", type=int, default=1, help="Nフレームに1回推論する")
    p.add_argument("--save-video", action="store_true", help="box+keypoint描画済み動画も保存する")
...
def main():
    ...
    model = load_checkpoint(args.weights, len(classes) + 1, num_keypoints, device=args.device)
    ...
    out_dir = (Path(args.out_dir) if args.out_dir
               else ROOT / "runs" / "detect_keypoint" / args.task / Path(args.weights).parent.name)
    ...
    for video in tqdm(videos, desc="推論"):
        ...
        result = run_on_video(model, video, classes, point_counts, args.device,
                               args.conf_thres, args.frame_stride, save_video_path)
