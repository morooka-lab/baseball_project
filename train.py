...
from utils.general import check_dataset, evaluation, outcome, tensorboard_log
...
def wbce_loss(y_true, y_pred):
    return (
        ...
        * (
            ((1 - y_pred) ** 2)
            * y_true
            * torch.log(torch.clamp(y_pred, min=1e-07, max=1))
            + (y_pred**2)
            * (1 - y_true)
            * torch.log(torch.clamp(1 - y_pred, min=1e-07, max=1))
        ).sum()
    )
    ...
    with torch.inference_mode():
        ...
        for batch_index, (X, y) in enumerate(pbar):
            ...
            pbar.set_description(
                "Val   loss: {:.6f}  |  TP: {}, TN: {}, FP1: {}, FP2: {}, FN: {}  |  Accuracy: {:.4f}, Precision: {:.4f}, Recall: {:.4f}".format(
...
def training_loop(
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for batch_index, (X, y) in enumerate(pbar):
            ...
            pbar.set_description(
                "Train loss: {:.6f}  |  TP: {}, TN: {}, FP1: {}, FP2: {}, FN: {}  |  Accuracy: {:.4f}, Precision: {:.4f}, Recall: {:.4f}".format(
            ...
            if batch_index % log_period == 0:
                with torch.inference_mode():
                    ...
                    torchvision.utils.save_image(
                        grid,
                        "{}/epoch_{}_batch{}.png".format(log_dir, epoch, batch_index),
                    )
        ...
        if epoch % 10 == 0:
            ...
            shutil.copyfile(cur_ckpt, latest_ckpt)  # ← ln コマンドの代わりにこれを使う
...
def parse_opt():
    ...
    parser.add_argument(
        "--data",
        type=str,
        default=ROOT / "data/match/test.yaml",
    ...
    parser.add_argument(
        "--project", default=ROOT / "runs/train", help="save results to project/name"
    )
    parser.add_argument(
        "--resume", action="store_true", help="whether load checkpoint for resume"
    )
...
def main(opt):
    ...
    train_loader = create_dataloader(
        train_path, imgsz, batch_size=batch_size, augment=True, shuffle=True
    )  # augment in training
